...
// ── Email normaliser ─────────────────────────────────────────
// Simple, consistent normalisation: lowercase + trim only.
// No Gmail dot-removal or other transforms that cause login mismatches.
...
exports.signup = async (req, res) => {
  ...
  // FIXED: always normalise email the same way as login
  ...
  try {
    ...
    try {
      ...
      // email_verified column missing (migration not run) — insert without it
    ...
    // DEV MODE — skip email, log in immediately
    if (devMode) {
    ...
    try {
      ...
      // Token table missing — auto-verify as fallback
...
exports.login = async (req, res) => {
  // FIXED: normalise email exactly like signup does — prevents "no account found" mismatch
  const email    = normaliseEmail(req.body.email)
  ...
  try {
    ...
    // email_verified NULL (no migration) → treat as verified
