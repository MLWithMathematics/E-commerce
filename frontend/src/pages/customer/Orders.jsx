...
import { RefreshCw, XCircle, Calendar, ChevronDown, ChevronUp, Package } from 'lucide-react'
...
function OrderCard({ order, expanded, onToggle, onCancel, onReorder, onReschedule }) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        <div className="flex-1">
          ...
          <div className="flex items-center gap-3 mt-1 text-sm text-[#6b7280]">
            ...
            <span>·</span>
            <span className="font-semibold text-[#1a1f2e]">${parseFloat(order.total).toFixed(2)}</span>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button onClick={onToggle} className="btn-ghost p-1.5 ml-1">
          ...
          <div className="space-y-3">
            {items.map(item => (
              <div key={item.id} className="flex items-center gap-3">
                ...
                <p className="text-sm font-semibold">${parseFloat(item.unit_price).toFixed(2)}</p>
              </div>
            ))}
          </div>
